...
-- Phase A/B/C: atomic RPCs for reliability

-- A new optional category column on cylinders
ALTER TABLE public.cylinders ADD COLUMN IF NOT EXISTS category text;

-- 1) find_or_create_cylinder: atomic upsert by barcode
...
-- Ensure barcode unique for ON CONFLICT to work
CREATE UNIQUE INDEX IF NOT EXISTS cylinders_barcode_unique ON public.cylinders(barcode);

-- 2) record_exchange: atomic partner-cylinder swap
...
-- 3) record_supplier_exchange: atomic
