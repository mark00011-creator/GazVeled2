...
CREATE TABLE IF NOT EXISTS public._live_validation_results (
...
DELETE FROM public._live_validation_results WHERE run_id = 'bf452a3-post-bypass';

CREATE OR REPLACE FUNCTION pg_temp.assert_pass(p_section text, p_name text, p_ok boolean, p_detail text)
RETURNS void LANGUAGE plpgsql AS $$
BEGIN
  INSERT INTO public._live_validation_results (run_id, section, test_name, status, detail) VALUES (
    'bf452a3-post-bypass', p_section, p_name, CASE WHEN p_ok THEN 'PASS' ELSE 'FAIL' END, p_detail
  );
END;
$$;
