...
export function BarcodeScanner({ onResult, onClose }: { onResult: (text: string) => void; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const ctrl = await reader.decodeFromVideoDevice(undefined, videoRef.current!, (res, _e, c) => {
          if (res && !stopped) {
            stopped = true;
    ...
    return () => { stopped = true; ctrlRef.current?.stop(); };
  }, [onResult]);

  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-black">
      <div className="flex items-center justify-between p-3 text-white">
        <div className="flex items-center gap-2"><Camera className="h-4 w-4" /> Vonalkód olvasás</div>
        <Button variant="ghost" size="sm" onClick={onClose}><X className="h-4 w-4" /></Button>
      </div>
      <video ref={videoRef} className="flex-1 object-cover" playsInline muted />
      {err && <div className="bg-destructive p-3 text-center text-sm text-destructive-foreground">{err}</div>}
