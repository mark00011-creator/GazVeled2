...
function QuickExchange() {
  ...
  async function complete() {
    if (!incoming || !outgoing || !partnerId) {
  ...
  const isNewCylFormValid =
    newCylForm.barcode.trim() && newCylForm.gasType && newCylForm.size && newCylForm.owner;
  ...
  return (
    <AppShell title="Gyors csere">
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2 pt-4">
              ...
              <Button
                onClick={saveNewCylinder}
                disabled={!isNewCylFormValid || newCylBusy}
                className="flex-1"
              >
                Mentés
              </Button>
          ...
          <div className="space-y-1">
            ...
            {(rentedCyls?.length ?? 0) === 0 && (
              <div className="text-xs text-destructive/80">
                Bérlet aktív, de a jelenlegi palack nincs hozzárendelve.
              </div>
            )}
          </div>
        </Card>
      ...
      {partnerId && (activeRentals?.length ?? 0) === 0 && (
        <Card className="mb-3 p-3 text-xs text-muted-foreground">
          Nincs aktív bérlet ennél a partnernél.
        </Card>
      )}
      ...
      {needsRentalQuestion && (
        <Card className="mb-3 border-primary/50 bg-primary/10 p-4">
          ...
          <div className="mb-2 text-xs text-muted-foreground">
            Jelenlegi bérelt palack:{" "}
            <span className="font-mono">{rentedCyls?.[0]?.barcode ?? "—"}</span> → új:{" "}
            <span className="font-mono">{outgoing!.barcode}</span>
          </div>
          <div className="flex gap-2">
            <Button
              className="flex-1"
      ...
      {incoming && outgoing && (
        <>
          ...
          <Button size="lg" className="w-full" disabled={busy} onClick={complete}>
            <Check className="mr-2 h-5 w-5" /> Csere rögzítése{" "}
