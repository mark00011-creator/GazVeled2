...
import { AlertTriangle, RefreshCw, Cylinder, Boxes, TrendingUp, FileText, Bell, Calendar, Gift, Package } from "lucide-react";

import { fmtDate, isRentalExpired, rentalTypeLabels, type RentalType } from "@/lib/labels";

import { daysUntil, formatExpiryWarning, formatInvoiceWarning, invoiceUrgency } from "@/lib/rental-billing";
...
function Dashboard() {
  ...
  const { data: stats, isLoading, isError } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: cyls, error: cylErr } = await supabase.from("cylinders").select("circulation,status,location_type,last_movement_at").eq("active", true);

      if (cylErr) throw cylErr;

      const { count: forced, error: forcedErr } = await supabase.from("exchanges").select("*", { count: "exact", head: true }).eq("is_forced_substitution", true);
      ...
      const { data: rentals, error: rentErr } = await supabase
        ...
        .select("id, monthly_fee, status, next_invoice_date, expiry_date, rental_type, partners(name)")
      ...
      const yearlyExpiring = activeRentals
        ...
        .filter((r) => (r.rental_type === "yearly" || !r.rental_type || r.rental_type === "free") && r.expiry_date)
      ...
      return {
        ...
        orderableSiad: by((c) => c.status === "empty" && c.location_type === "warehouse_empty" && c.circulation === "siad"),

        orderableOwn: by((c) => c.status === "empty" && c.location_type === "warehouse_empty" && c.circulation === "own"),
  ...
  if (isError) {
    ...
    return (
      ...
      <AppShell title="Áttekintés">
        ...
        <div className="py-8 text-center text-sm text-destructive">Dashboard betöltése sikertelen</div>
    ...
    <AppShell title="Áttekintés">
      ...
      <div className="mb-4 grid grid-cols-2 gap-3">
        ...
        <Card className="p-4">
          ...
          <div className="flex items-center gap-2 text-xs text-muted-foreground"><FileText className="h-3.5 w-3.5" /> Aktív bérletek</div>
        ...
        <Card className="p-4">
          ...
          <div className="flex items-center gap-2 text-xs text-muted-foreground"><Cylinder className="h-3.5 w-3.5" /> Kint lévő bérpalackok</div>
        ...
        <Card className="p-4">
          ...
          <div className="flex items-center gap-2 text-xs text-muted-foreground"><TrendingUp className="h-3.5 w-3.5" /> Havi bérleti bevétel</div>

          <div className="mt-1 text-xl font-bold">{(stats?.monthlyRevenue ?? 0).toLocaleString("hu-HU")} Ft</div>

        </Card>

        <Card className="p-4">

          <div className="flex items-center gap-2 text-xs text-muted-foreground"><Bell className="h-3.5 w-3.5" /> 5 napon belül számlázandó</div>
        ...
        <Card className="mb-4 p-4">
          ...
          <div className="space-y-2">
            ...
            {stats!.yearlyExpiring.map((r) => {
              ...
              const tone = urgency === "red" ? "border-destructive/50 bg-destructive/10" : "border-warning/50 bg-warning/10";
                  ...
                  <div className={`rounded-lg border p-3 text-sm ${tone}`}>
                    ...
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <div className="flex gap-1">
                        ...
                        {isRentalExpired(r.expiry_date) && <Badge variant="destructive">LEJÁRT</Badge>}

                        <Badge variant="outline" className="font-mono text-[10px]">{rentalNumber(r.id)}</Badge>
                    ...
                    <div className={`mt-1 text-xs ${isRentalExpired(r.expiry_date) ? "font-medium text-destructive" : "text-muted-foreground"}`}>
              ...
              return (
                ...
                <Link key={r.id} to="/rentals/$id" params={{ id: r.id }}>
                  ...
                  <div className={`rounded-lg border p-3 text-sm ${tone}`}>
                    ...
                    <div className="mt-1 text-xs text-muted-foreground">Esedékes: {fmtDate(r.next_invoice_date)}</div>
              ...
              <Link key={r.id} to="/rentals/$id" params={{ id: r.id }}>
                ...
                <div className="rounded-lg border border-border/60 p-3 text-sm transition-colors hover:bg-accent/50">
                  ...
                  {r.expiry_date && (
                    ...
                    <div className="mt-1 text-xs text-muted-foreground">Lejárat: {fmtDate(r.expiry_date)}</div>
